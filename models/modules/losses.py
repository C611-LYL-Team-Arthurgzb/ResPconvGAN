...
class TVLoss2(nn.Module):
    ...
    def forward(self,x):
        ...
        loss = 1.0*(h_variance + w_variance)
        print("jjjkkkjj",self.TVLoss_weight,loss,h_variance , w_variance,(h_variance + w_variance))
        return loss
...
class TVLoss(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(h_tv,count_h,count_w,w_tv,h_tv / count_h ,w_tv / count_w)
        # print("ssssss",1.0 * 2.0 * (h_tv / count_h + w_tv / count_w) / 1.0)
        # print("llllll",self.tv_loss_weight,x[:, :, 1:, :].shape,x[:, :, 1:, :].size(1),x[:, :, 1:, :].size(2),x[:, :, 1:, :].size(3))
        # print("ffff",1.0 * 2 * (h_tv / count_h + w_tv / count_w) / bz)
        return 1.0 * 2 * (h_tv / count_h + w_tv / count_w) / bz
